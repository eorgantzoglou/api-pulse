...
1. GitHub → Settings → Developer settings → Personal access tokens → **Fine-grained tokens** → Generate new token. Repository access: **only this repository**. Permissions: **Contents: read and write** — nothing else.

   *Revised during execution.* The original plan called for a classic `repo`-scope token with no expiration, reasoning that a fine-grained token's one-year cap "would expire mid-absence, silently killing the pipeline." That reasoning is wrong, and the final review said so: an expired token fails the `checkout` step, `if: ${{ !success() }}` fires, and the `github-script` step uses `GITHUB_TOKEN` — it needs no checkout and already holds `issues: write` — so it still opens the issue. The expiry is loud, not silent. Meanwhile a classic `repo` token grants write access to *every* repository on the account and is persisted into the runner's git config, in the same job that installs dependencies from PyPI. Scope it down and diary the renewal.
...
- `pytest` is green: **101 tests** (the original estimate was 68; review rounds added
  33), no network access.
...
## Where the shipped code diverges from this plan

Four revision notes above record changes made *to* this plan during execution. Three
further divergences went the other way — the code is right and this document was not
rewritten to match, because the plan's version is still the clearer explanation of intent:

- **`cli.py` writes via a single `_write_all_json`**, not the `_write_json` / `_promote` /
  `_discard` trio shown here. Same staging-then-promote semantics, fewer moving parts.
- **`probe.py`'s `guarded()` carries an outer last-resort `try/except`** that this plan's
  code omits, so `probe_all` is total by construction rather than by argument.
- **A third circuit breaker exists that this plan never specified**:
  `check_category_continuity` in `aggregate.py`. The final review found that an upstream
  category rename re-mints every id in that category, orphaning its history and resetting
  confirmed-dead entries to healthy, with no breaker and no signal. It was first specified
  as a global 10% id-overlap ratio; that could not fire on any single-category rename in a
  1,752-entry catalogue (the largest category is 9.08%), so it is now per-category: the run
  aborts when a category present in the last good `catalog.json` loses every one of its ids.
